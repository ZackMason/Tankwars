...
class ZSprite(pygame.sprite.Sprite):
    ...
    def sineUpdate(self,player):
        self.x += math.cos(self.start_y/30)* 25
        self.y += self.dy
...
class ZPlayer(pygame.sprite.Sprite):
    ...
    def classChange(self, Type):
        if Type == 1:
            self.image = pygame.image.load('berserk.png').convert_alpha()
            self.Class = 'berserk'
        if Type == 2:
            self.image = pygame.image.load('stormtrooper.png').convert_alpha()
            self.Class = 'StormTrooper'
...
spMissiles = pygame.sprite.Group()
spMissiles0 = pygame.sprite.Group()
...
Sgun = weapons.add(Gun('pwrgun.png', random.randint(100,1000), random.randint(50, 600), 100, 'sine'))

font = pygame.font.Font(None, 48)

#Counters for firing/spawning time
firing = 0
fireTime = 0
firing2 = 0
fireTime2 = 0
...
while a == 1:
    controll = ZSprite('zombControlls.png', 0, 0, 0, 0)
    menu = ZSprite('Start.png', 0, 0, 0, 0)
    if b == 0:
        screen.blit(menu.image, menu.rect)
    if b == 1:
        screen.blit(controll.image, controll.rect)
...
while True:
    if state == "play":
        ...
        #On-Screen Text
        drawText('CrossBower: %s Kills: %s Ammo: %s' % (tank.health, tank.score, tank.pAmmo), font, screen,10,0)
        drawText2('Machine-Gunner: %s Kills: %s Ammo: %s' % (soldier.health, soldier.score, soldier.pAmmo), font, screen, 10, 30)
        ...
        for Boss in bosses:
            ...
            if Boss.hp <= 0:
                state = "class"
                ZMode = 1
                Difficulty = 9999999999999999999999999999
                create = 1
                lvl += 1
        ...
        collision02 = pygame.sprite.groupcollide(spMissiles, baddies, False, True)
        collision03 = pygame.sprite.groupcollide(spMissiles0, baddies, False, True)
        collision04 = pygame.sprite.groupcollide(spMissiles, bosses, True, False)
        collision05 = pygame.sprite.groupcollide(spMissiles0, bosses, True, False)
        ...
        if len(collision03) >= 1:
            tank.score += 1
            collision03 = 0

        if len(collision02) >= 1:
            soldier.score += 1
            collision02 = 0

        if len(collision04) >= 1:
            Boss.hp -= 1
            if Boss.hp == 0:
                soldier.score += 1000
            collision04 = 0

        if len(collision05) >= 1:
            Boss.hp -= 1
            if Boss.hp == 0:
                soldier.score += 1000
            collision05 = 0
        ...
        if firing == 1:
            if tank.Class == None:
                if pygame.time.get_ticks() - fireTime > 35:
                    missiles.add(ZSprite('bolt.png', tank.x + 40, tank.y, random.randint(-2,2) ,-10))
                    fireTime = pygame.time.get_ticks()
            if tank.Class == 'berserk':
                missiles.add(ZSprite('bullet.png', tank.x + 33, tank.y, random.randint(-1,1) ,-10))
                missiles.add(ZSprite('bullet.png', tank.x + 5, tank.y, random.randint(-1,1) ,-10))
                fireTime = pygame.time.get_ticks()
            if tank.Class == 'StormTrooper':
                missiles.add(ZSprite('blast.png', tank.x + 33, tank.y - 22, random.randint(-1,1) ,-10))

        if firing2 == 1:
            if pygame.time.get_ticks() - fireTime2 > 35:
                if soldier.Class == None:
                    missiles2.add(ZSprite('bullet.png', soldier.x + 33, soldier.y, random.randint(-1,1) ,-20))
                    fireTime2 = pygame.time.get_ticks()
                if soldier.Class == 'berserk':
                    missiles2.add(ZSprite('bullet.png', soldier.x + 33, soldier.y, random.randint(-1,1) ,-20))
                    missiles2.add(ZSprite('bullet.png', soldier.x + 5, soldier.y, random.randint(-1,1) ,-20))
                    fireTime2 = pygame.time.get_ticks()
                if soldier.Class == 'StormTrooper':
                    missiles2.add(ZSprite('blast.png', soldier.x + 33, soldier.y - 22, random.randint(-1,1) ,-20))
        ...
        if firing3 == 1:
            if pygame.time.get_ticks() - fireTime3 > 25:
                if soldier.spWep == 'Flamer':
                    if soldier.pAmmo > 0:
                        soldier.pAmmo -= 1
                        missiles2.add(ZSprite('napalm.png', soldier.x + 33, soldier.y, random.randint(-2,2), -10 + random.randint(-2,2)))
        ...
        if firing4 == 1:
            if pygame.time.get_ticks() - fireTime4 > 25:
                if tank.spWep == 'Flamer':
                    if tank.pAmmo > 0:
                        tank.pAmmo -= 1
                        missiles.add(ZSprite('napalm.png', tank.x + 40, tank.y, random.randint(-2,2), -5 + random.randint(-1,1)))
                        fireTime4 = pygame.time.get_ticks()

        if firing5 == 1:
            if pygame.time.get_ticks() - fireTime5 > 150:
                if soldier.spWep == "sine":
                    if soldier.pAmmo > 0:
                        soldier.pAmmo -= 1
                        spMissiles.add(ZSprite('sineshot.png', soldier.x + 33, soldier.y, 0, -5))
                        soldier.start_x = soldier.x
                        fireTime5 = pygame.time.get_ticks()

        if firing6 == 1:
            if pygame.time.get_ticks() - fireTime6 > 150:
                if tank.spWep == 'sine':
                    if tank.pAmmo > 0:
                        tank.pAmmo -= 1
                        spMissiles0.add(ZSprite('sineshot.png', tank.x + 40, tank.y, 0, -5))
                        tank.start_x = tank.x
                        fireTime6 = pygame.time.get_ticks()
        ...
        if ZMode == 2:
            if create == 1:
                Boss = bosses.add(ZZombies('MyBoss.png', 500, -100, 3, 3, 1000))
                create = 0
        ...
        screen.fill(textColor)
        drawText("1, 3 = Dual Machine-Gunner ", font, screen, 20, (heigth/2)/2)
        drawText("2, 4 = Storm Trooper", font, screen, 20, ((heigth/2)/2)+ 30)
        drawText("To change Crossbow Charecter press 1 or 2",font,screen, 20, heigth / 2)
        drawText("To change Machine Gun Charecter press 3 or 4",font,screen, 20, (heigth / 2) + 30)
        for event in pygame.event.get():
            if event.type == QUIT:
                pygame.quit()
                break
                ...
                if event.key == K_1:
                    ...
                    soldier.classChange(1)

                elif event.key == K_4:
                    soldier.classChange(2)
